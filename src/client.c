...
void send_length(int pid, int length) 
{
	// Send the length byte by byte
...
int	check_av(int ac, char **av)
{
	...
	if (ac != 3 || !av[2][0])
		ft_print_error("Error arguments, please try again.\n", 2, EXIT_FAILURE);
	pid = ft_atoi(av[1]);
	if (pid == 0)
		ft_print_error("Error: PID 0 is reserved for the kernel.\n", \
			2, EXIT_FAILURE);
	i = -1;
	while (av[1][++i])
	{
		if (!(ft_isdigit(av[1][i])) || pid < 0)
			ft_print_error("Error: a PID only has positive decimal numbers.", \
				2, EXIT_FAILURE);
	}
	return (pid);
}
